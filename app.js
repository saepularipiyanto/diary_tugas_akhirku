...
const ADMIN_USERNAME = 'saepul.aripiyanto@gmail.com';
const ADMIN_PASSWORD = 'UINJ@k@r7@2026';
...
function initUsersDatabase() {
  ...
  // Ensure admin exists (migrate legacy 'admin' account if present)
  const legacyAdminIdx = usersArray.findIndex(u => u.username === 'admin' && u.role === 'admin');
  const newAdminIdx = usersArray.findIndex(u => u.username === ADMIN_USERNAME);

  if (legacyAdminIdx !== -1) {
    if (newAdminIdx !== -1 && newAdminIdx !== legacyAdminIdx) {
      usersArray.splice(legacyAdminIdx, 1);
    } else {
      usersArray[legacyAdminIdx].username = ADMIN_USERNAME;
      usersArray[legacyAdminIdx].password = ADMIN_PASSWORD;
    }
  }

  const adminUser = usersArray.find(u => u.role === 'admin');
  if (!adminUser) {
    usersArray.push({
  ...
  } else if (adminUser.username === ADMIN_USERNAME) {
  ...
  usersArray.forEach(u => {
    if (!u.role) {
      u.role = (u.username === ADMIN_USERNAME) ? 'admin' : 'student';
    }
  });
...
function initAppState() {
  ...
  if (isAdmin) {
    ...
    appState = { role: 'admin', username: userMeta.username };
    return;
  }
...
function renderAdminAverageProgress(students) {
  const pctEl = document.getElementById('admin-avg-progress-pct');
  const ringEl = document.getElementById('admin-avg-progress-ring');
  if (!pctEl || !ringEl) return;
  ...
  pctEl.innerText = `${avgProgress}%`;

  const circumference = 126;
  ringEl.style.strokeDashoffset = circumference - (circumference * avgProgress) / 100;
}
...
function renderAdminComponents() {
  ...
  renderAdminAverageProgress(students);
      ...
      <tr>
        ...
        <td style="text-align: center;">
          <div style="display: flex; gap: 0.35rem; justify-content: center; flex-wrap: wrap;">
            <button class="btn btn-secondary btn-sm btn-icon-only" onclick="inspectStudent('${student.username}')" title="Lihat Detail">
              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7Z"/><circle cx="12" cy="12" r="3"/></svg>
            </button>
            <button class="btn btn-secondary btn-sm btn-icon-only" onclick="adminEditStudent('${student.username}')" title="Edit">
              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg>
            </button>
            <button class="btn btn-secondary btn-sm btn-icon-only" onclick="adminDeleteStudent('${student.username}')" title="Hapus" style="color: var(--danger);">
              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>
            </button>
          </div>
        </td>
...
function resetAdminStudentModal() {
  const originalInput = document.getElementById('admin-student-original-username');
  const usernameInput = document.getElementById('admin-student-username-input');
  const passwordInput = document.getElementById('admin-student-password-input');
  const passwordHint = document.getElementById('admin-student-password-hint');
  ...
  if (passwordInput) {
    ...
    passwordInput.placeholder = 'Contoh: password123';
  }
  if (passwordHint) passwordHint.style.display = 'none';
  if (document.getElementById('admin-student-name-input')) {
    document.getElementById('admin-student-name-input').value = '';
  }
  if (document.getElementById('admin-student-nim-input')) {
    document.getElementById('admin-student-nim-input').value = '';
  }
  if (document.getElementById('admin-student-major-input')) {
    document.getElementById('admin-student-major-input').value = '';
  }
  if (modalTitle) modalTitle.innerText = 'Tambah Mahasiswa Baru';
  if (saveBtn) saveBtn.innerText = 'Tambah Mahasiswa';
}

window.adminEditStudent = function(username) {
  const users = JSON.parse(localStorage.getItem(USERS_KEY) || '[]');
  const user = users.find(u => u.username === username && u.role !== 'admin');
  if (!user) return;

  document.getElementById('admin-student-original-username').value = user.username;
  document.getElementById('admin-student-username-input').value = user.username;
  document.getElementById('admin-student-password-input').value = '';
  document.getElementById('admin-student-password-input').required = false;
  document.getElementById('admin-student-password-input').placeholder = 'Kosongkan jika tidak diubah';
  document.getElementById('admin-student-name-input').value = user.name;
  document.getElementById('admin-student-nim-input').value = user.nim;
  document.getElementById('admin-student-major-input').value = user.major;

  const passwordHint = document.getElementById('admin-student-password-hint');
  if (passwordHint) passwordHint.style.display = 'block';

  document.getElementById('modal-student-title').innerText = 'Edit Data Mahasiswa';
  document.getElementById('modal-student-save-btn').innerText = 'Simpan Perubahan';
  document.getElementById('modal-student').classList.add('active');
};

window.adminDeleteStudent = function(username) {
  const users = JSON.parse(localStorage.getItem(USERS_KEY) || '[]');
  const user = users.find(u => u.username === username);
  if (!user || user.role === 'admin') return;

  if (!confirm(`Apakah Anda yakin ingin menghapus akun mahasiswa "${user.name}" (@${username})?\n\nSemua data bimbingan, log, dan progress mahasiswa ini akan dihapus permanen.`)) {
    return;
  }

  const updatedUsers = users.filter(u => u.username !== username);
  localStorage.setItem(USERS_KEY, JSON.stringify(updatedUsers));
  localStorage.removeItem(`diary_tugas_akhir_state_${username}`);
  ...
  alert(`Akun mahasiswa ${user.name} (@${username}) berhasil dihapus.`);
};
...
function setupAdminEventListeners() {
  ...
  if (formStudent) {
    formStudent.addEventListener('submit', (e) => {
      e.preventDefault();
      const originalUsername = document.getElementById('admin-student-original-username').value;
      const isEditMode = Boolean(originalUsername);
      const username = document.getElementById('admin-student-username-input').value.trim().toLowerCase();
      const password = document.getElementById('admin-student-password-input').value;
      const name = document.getElementById('admin-student-name-input').value.trim();
      const nim = document.getElementById('admin-student-nim-input').value.trim();
      const major = document.getElementById('admin-student-major-input').value.trim();

      const users = JSON.parse(localStorage.getItem(USERS_KEY) || '[]');

      if (isEditMode) {
        const userIdx = users.findIndex(u => u.username === originalUsername && u.role !== 'admin');
        if (userIdx === -1) {
        ...
        users[userIdx] = {
          ...users[userIdx],
          username,
          password: password || users[userIdx].password,
          name,
          nim,
          major
        };
        ...
        const stateKey = `diary_tugas_akhir_state_${originalUsername}`;
        ...
        if (stateStr) {
          try {
            ...
            if (state.profile) {
              ...
              state.profile.nim = nim;
              state.profile.major = major;
            }
            if (username !== originalUsername) {
              localStorage.setItem(`diary_tugas_akhir_state_${username}`, JSON.stringify(state));
              localStorage.removeItem(stateKey);
            } else {
              localStorage.setItem(stateKey, JSON.stringify(state));
            }
          } catch (err) {
            console.error('Gagal memperbarui state mahasiswa:', err);
          }
        }

        alert(`Data mahasiswa ${name} (@${username}) berhasil diperbarui!`);
      } else {
        ...
        if (!password) {
          alert('Kata sandi wajib diisi untuk mahasiswa baru.');
          return;
        }
